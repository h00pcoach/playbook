	...
	for(var i=0;i<moves[sel].frame.length;++i)
	{
		...
		if(v.category=='Arrow')
		{
			if(parseInt(v.type)!=5){
				...
				// court tap detection
				// grp.on('tap', function (evt)
				// {
				// 	// console.log('court tapped: ', evt);
				// });
...
{
	// console.log('playLoading: ', frm, to);
...
{
	console.log('nextFrame NXT? ', nxt);
	console.log('nextFrame LST? ', lst);

	console.log('nextFrame layer? ', layer);
		...
		for(var k=0;k<moves[lst].frame.length;++k)
		{
			if(node.getId()==moves[lst].frame[k].id)
			{
				var tween = new Kinetic.Tween({
					...
					onFinish: function()
					{
						console.log('Finish tween and layer? ', layer);
						if(!nxt && layer)
						{
	...
	if(moves.length>1 && moves.length>lst)
	{
		...
		playing=setInterval(function()
		{
			if(plItems==0)
			{
				...
				console.log('playNow layer? ', layer);
...
$(document).ready(function(){
  ...
  //   	}
	...
	$(document).on('touchend', '#container', function(e)
	{
		...
		if (event.type == "click") docClick = true;

		//// console.log('doc touchend! docClick? ', docClick);
...
//			//// console.log('drgg is what? ', drgg);
	...
	{
		console.log('Mousedown! playLayer? ', playLayer);
		if(playLayer && playLayer.children)
		{
		...
		// else {
        //
		// 	console.log('moves value? ', $('#moves').val());
		// 	playNow(1, false);
        //
		// }
	});
	...
	{
		console.log('Touchdown! playLayer? ', playLayer);
...
{
	// console.log('AddObjects Acetrik! ', tag);
	if(tag){
		if(tag!='Arrow'){
			$('#moves option').each(function(i,e){
