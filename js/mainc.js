...
//        }
        ...
        $.ajax({url:'load.php',data:{id:c,user_id:g},dataType:'json',success:function(n)
        {
            ...
            moves=n.play.frames;filenm=n.play.name;fileid=n.id;$('#userid').val(g);user.uped=n.uped;user.downed=n.downed;$('#id').val(n.id);$('#tags').val(n.tags);$('#speed').val(n.play.speed);$('#moves').html('');
            for(var i in moves){
		...
		($.ajax({type:'POST',url:'save.php',
			...
			success:function(d){
				($('status',d).text()=='\x31')?(
					...
					,$('#lrg').hide('slow'),$('#savename').val(''),$('#id').val($('id',d).text())):0;alert($('msg',d).text());showLoading(true)
			}
		})):
		(
			$.ajax({
	...
	function addObjects(x,c,g,n)
		{
			...
			// console.log('addObjects main! string x: ' + JSON.stringify(x) + ' c: ' + c + ' g: ' + g + ' n: ' + n);
			if(c){
				if(c !== 'Arrow')
				{
					// //// console.log('c != Arrow!');
