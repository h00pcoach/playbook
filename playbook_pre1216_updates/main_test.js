...
//        }
        ...
        $.ajax({url:'load.php',data:{id:c,user_id:g},dataType:'json',success:function(n){
            if(!n.play){alert('There is an error parsing the saved data, Administrator will be notified. \nSorry for inconvenience.');$.ajax({url:'email.php',data:{sender:'user',receiver:'bastasys.info@gmail.com',msg:'ERRROR',purl:location.href},success:function(d){location.href='play.php'}});return false}halfcourt=(n.play.halfcourt=="true"?true:false);hash=n.play.skin;moves=n.play.frames;filenm=n.play.name;fileid=n.id;$('#userid').val(g);user.uped=n.uped;user.downed=n.downed;$('#id').val(n.id);$('#tags').val(n.tags);$('#speed').val(n.play.speed);$('#moves').html('');
            for(var i in moves){
                $('#moves').append($('<option>',{value:i,text:moves[i].name}));
            }
            var o=($('skin#'+hash,d));
            showLoading(true);loadElements(true);$('#court').val(halfcourt?1:0)
        },error:function(d){alert('Your internet connection is lost or the server encountered an error.');loadElements();filenm='Name';fileid='';$('#userid').val(user.id);$('#id').val('')}})
    }else{
